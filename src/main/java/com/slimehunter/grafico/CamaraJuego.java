...
public class CamaraJuego {
	...
	//	this.camara.position.y = MathUtils.lerp(this.camara.position.y, objetivoY, 0.1f);
		...
		float mediaAlto = this.altoVista * this.camara.zoom / 2f;

		this.camara.position.x = MathUtils.clamp(this.camara.position.x, this.limiteIzquierdo + mediaAncho,
				this.limiteDerecho - mediaAncho);
		this.camara.position.y = MathUtils.clamp(this.camara.position.y, this.limiteInferior + mediaAlto,
