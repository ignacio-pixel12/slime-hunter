...
public class EnemigoSaltarin extends EntidadDinamica {
    ...
    private void updateCaminando(float delta) {
        ...
        if (this.yendoDerecha) {
            this.mover(Constantes.SLIME_VELOCIDAD * 0.3f);
        } else {
            this.mover(-Constantes.SLIME_VELOCIDAD * 0.3f);
        }

        this.temporizadorSalto += delta;
        if (this.temporizadorSalto >= INTERVALO_SALTO && this.estaEnElSuelo()) {
            this.saltar();
            this.temporizadorSalto = 0f;
        }
    ...
    private String getNombreAnimacion() {
        return this.muerto ? "Saltar" :
            this.getTablaEstados().getEstadoActual() == EstadoAnimacion.RECIBIENDO_DANO ? "hit" : "Saltar";
    }
    ...
    private float getEscala() { return Constantes.SLIME_ESCALA; }
